...
grid.extent2<-data.frame('lon'=rep(-155,100),'lat'=rep(51,100),'doy'=seq(min(yfsub$doy),max(yfsub$doy),length=100),
                         'year'=rep(2002,100),'bottom_depth'=rep(median(yfsub$bottom_depth,na.rm=TRUE),100),
                         'reg.SST'=mean(yfsub$reg.SST[yfsub$reg.SST<4.143]),'th'="TRUE")
...
grid.extent2$reg.SST<-mean(yfsub$reg.SST[yfsub$reg.SST>4.143])
...
polygon(c(grid.extent2$doy,rev(grid.extent2$doy)),c(grid.extent2$pred2.l,rev(grid.extent2$pred2.u)),
        col=warmcol,lty=0)
legend('topleft',legend=c(expression(paste(mu, '(<4.143'^0,' C)')),
                          expression(paste(mu,'(>4.143'^0,' C)'))),
