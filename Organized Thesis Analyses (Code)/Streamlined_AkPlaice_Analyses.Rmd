...
unit$salinity<-as.numeric(median(aplarv.ctd$salinity,na.rm=T))
...
unis$temperature<-as.numeric(median(aplarv.ctd$temperature,na.rm=T))
...
sal<-ggplot(grid.clip.s,aes(x=salinity,y=pred))+geom_point(color='grey')+geom_smooth(method="loess")+
  ...
  scale_x_continuous(breaks=round(seq(min(grid.clip.s$salinity),max(grid.clip.s$salinity),by=0.2),1))
