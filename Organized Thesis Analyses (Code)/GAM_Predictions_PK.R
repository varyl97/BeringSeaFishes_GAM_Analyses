##GAM Predictions: Walleye Pollock
#the following code produces predictions and predictive figures based off the best model 
#for Walleye Pollock, the best model is the threshold geography model wherein spawning geography 
#varies differently above and below 2.048 *C. The best phenology model was also the threshold formulation
  #where the threshold temperature was 2.009 *C. 

# Preliminary Data Loading and Formatting ---------------------------------

#link pksub to regional indices:
pksub<-read.csv(file='./Ichthyo Data/Cleaned_Cut_PkEggs.csv',header=TRUE,check.names=TRUE) #for egg GAMs
pklarv.ctd<-read.csv(file='./Ichthyo Data/Cleaned_Cut_PkLarv_wCTD.csv',header=TRUE,check.names=TRUE) #for larval GAMs
...
temps.in<-readRDS("./GAM Models/pk_egg_temps_in_phe.rds") 
...
lv.2d<-readRDS("./GAM Models/pk_larvae_2d.rds") #only using lv.base and lv.2d in this code. 
...
grid.extent2<-data.frame('lon'=rep(-155,100),
                         ...
                         'year'=rep(2008,100),'bottom_depth'=rep(median(pksub$bottom_depth,na.rm=TRUE),100))#setting up another clean grid on which to predict
...
windows(height=15,width=15)
par(mai=c(1,1,0.5,0.9))
plot(grid.extent2$doy,grid.extent2$pred,main='Walleye Pollock Base Phenology, Eggs',type='l',
     ylab=expression(paste("(log(C/(10m"^2,')+1)')),xlab='Day of Year',cex.lab=1,
     cex.axis=1,cex.main=1,cex.axis=0.9,xlim=range(pksub$doy),
...
grid.extent3<-data.frame('lon'=rep(-155,100),'lat'=rep(51,100),'doy'=seq(min(pksub$doy),max(pksub$doy),length=100),
                         'year'=rep(2008,100),'bottom_depth'=rep(median(pksub$bottom_depth,na.rm=TRUE),100),
                         'reg.SST'=mean(pksub$reg.SST[pksub$reg.SST<2.009]),'th'="TRUE")
...
grid.extent3$reg.SST<-mean(pksub$reg.SST[pksub$reg.SST>2.009])
...
polygon(c(grid.extent3$doy,rev(grid.extent3$doy)),c(grid.extent3$pred2.l,rev(grid.extent3$pred2.u)),
        col=warmcol,lty=0)
legend('bottomright',legend=c(expression(paste(mu, '(<2.009'^0,' C)')),
                          expression(paste(mu,'(>2.009'^0,' C)')),'90% CI','90% CI'),
