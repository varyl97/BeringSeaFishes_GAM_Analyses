...
lv.2d<-readRDS("./GAM Models/rx_larvae_2d.rds")#only using lv.base and lv.2d in this code. 
...
grid.extent$reg.SST<-mean(rxsub$reg.SST[rxsub$reg.SST<2.285]) #threshold temp chosen by AIC values
grid.extent$th<-"TRUE"
...
grid.extent$reg.SST<-mean(rxsub$reg.SST[rxsub$reg.SST>2.285])
...
grid.extent2<-data.frame('lon'=rep(-155,100),'lat'=rep(51,100),'doy'=seq(min(rxsub$doy),max(rxsub$doy),length=100),
                         'year'=rep(2008,100),'bottom_depth'=rep(median(rxsub$bottom_depth,na.rm=TRUE),100),
                         'reg.SST'=mean(rxsub$reg.SST[rxsub$reg.SST<2.285]),'th'="TRUE")
...
grid.extent2$reg.SST<-mean(rxsub$reg.SST[rxsub$reg.SST>2.285])
...
for(k in 1:nrow(grid.extent)){
  dist<-distance.function(grid.extent$temperature[k],grid.extent$salinity[k],
                          rxlarv.ctd$temperature,rxlarv.ctd$salinity)
  grid.extent$dist[k]<-min(dist)
}
...
image.plot(tempd,sald,t(matrix(grid.extent$pred,nrow=length(sald),ncol=length(tempd),byrow=T)),
           ...
           xlim=range(rxlarv.ctd$temperature,na.rm=T),ylim=range(rxlarv.ctd$salinity,na.rm=T),
