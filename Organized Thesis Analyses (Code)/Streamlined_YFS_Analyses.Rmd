...
aic.yflarv<-data.frame('model'=c('Base','Add Sal','Add Temp',
...
plot(c(1:5),aic.yflarv$AIC_value,main='YFS',
     col=hcl.colors(5,"Zissou 1"),
     pch=19,cex=2,ylab='AIC Score',xlab='')
grid(nx=5,ny=14,col="lightgray")
text(c(1:5),aic.yflarv$AIC_value,labels=round(aic.yflarv$AIC_value),pos=c(4,1,3,2,2))
...
#write.csv(grid.clip,'Yfs_larv_clip_60.csv')
```

So this percentage (percbi, 46.7%) is the percentage of the total grid extent within which 60% of the predicted log(CPUE+1) lies. I.e., 46.7% percent of the whole grid extent encompasses 60% of the predicted observations. 
...
for(k in 1:nrow(unit)){
  unit$dist[k]<-unit$temperature[k]-yflarv.ctd$temperature[k]
}

unit$year<-as.numeric(2005)
unit$lon<-as.numeric(median(yflarv.ctd$lon))
unit$lat<-as.numeric(median(yflarv.ctd$lat))
unit$doy<-as.numeric(median(yflarv.ctd$doy,na.rm=TRUE))
unit$bottom_depth<-as.numeric(median(yflarv.ctd$bottom_depth,na.rm=TRUE))
unit$salinity<-31
...
for(k in 1:nrow(unis)){
  unis$dist[k]<-unis$salinity[k]-yflarv.ctd$salinity[k]
}

unis$year<-as.numeric(2005)
unis$lon<-as.numeric(median(yflarv.ctd$lon))
unis$lat<-as.numeric(median(yflarv.ctd$lat))
unis$doy<-as.numeric(median(yflarv.ctd$doy,na.rm=TRUE))
unis$bottom_depth<-as.numeric(median(yflarv.ctd$bottom_depth,na.rm=TRUE))
unis$temperature<-11
unis$pred<-predict(lv.2d,newdata=unis)
unis$temperature<-11.25
unis$pred.1<-predict(lv.2d,newdata=unis)
unis$temperature<-12
unis$pred.2<-predict(lv.2d,newdata=unis)
unis$temperature<-10.25
...
bivar<-ggplot(grid.clip)+geom_tile(aes(x=salinity,y=temperature,fill=pred))+theme_bw()+
  ...
  geom_vline(xintercept=31.6,linetype="solid",color="grey48")+
  geom_vline(xintercept=31.75,linetype="twodash",color="grey48")+
  geom_vline(xintercept=32.6,linetype="longdash",color="grey48")+
  geom_vline(xintercept=30.5,linetype="dotted",color="grey48")+
  geom_vline(xintercept=30,linetype="dotdash",color="grey48")+
  geom_hline(yintercept=11,linetype="dashed",color="black")+
  geom_hline(yintercept=11.25,linetype="solid",color="grey48")+
  geom_hline(yintercept=12,linetype="twodash",color="grey48")+
  geom_hline(yintercept=10.25,linetype="longdash",color="grey48")+
  geom_hline(yintercept=9.5,linetype="dotted",color="grey48")+
  geom_hline(yintercept=4.25,linetype="dotdash",color="grey48")+
...
91.9% of the salinity-temperature area contains 60% of the total predicted log(CPUE+1). 
